...
~~기존 POST /api/generate는 그대로 유지 (레거시 클라이언트·폴백용).~~
**2026-09-20 삭제.** 프론트 전환 후 호출처가 0이 된 채 남아 있었고, 초대 게이트를 푸는 날
아무도 안 보는 생성 비용 창구가 열리는 문제가 있어 라우트·`lib/analyze.ts`와 함께 제거했다.
GET /api/analyze가 쿼터 조회를 이어받는다.
