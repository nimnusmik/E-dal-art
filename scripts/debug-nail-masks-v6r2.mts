/**
 * nail-masks.mts의 "6r2"(순환 라플라시안 통제점 스무딩 + 스플라인 조밀화 +
 * 첨단 확장) 마스크가 실제 손톱판을 정확히 덮는지 육안으로 확인하는 게이트1
 * 시트 생성기 — debug-nail-masks-v6r.mts와 동일한 로직이지만 출력 파일명을
 * v6r2-shape-check.png로 분리해, 기존 v6r-shape-check.png(통제점 스무딩 이전
 * 증거)를 덮어쓰지 않고 나란히 비교할 수 있게 한다.
 *   npx tsx scripts/debug-nail-masks-v6r2.mts
 */
...
async function run() {
  ...
  await sharp({ create: { width: sheetW, height: sheetH, channels: 4, background: { r: 20, g: 20, b: 20, alpha: 1 } } })
    ...
    .png()
    .toFile(resolve(OUT_DIR, 'v6r2-shape-check.png'));
  console.log('게이트1 확인용(6r) 손가락별 확대 시트:', resolve(OUT_DIR, 'v6r2-shape-check.png'));
}
run();
