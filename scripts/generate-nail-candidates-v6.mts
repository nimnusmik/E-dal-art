/**
 * 히어로 네일아트 후보 6차 라운드 — 2단계: 1단계(generate-nail-texture-v6.mts)
 * 텍스처를 nail-masks.mts의 "재트레이싱한 다각형 마스크"(타원 아님, 2026-08-04
 * 6차 수정)에 결정론적으로 매핑·재조명한다. 매핑·재조명 로직 자체는 4~5차와
 * 동일(사용자 승인된 품질) — 이번에 바뀐 것은 (1) 마스크가 다각형이라 손톱판
 * 바깥 피부에 디자인이 새지 않는다는 것, (2) SAMPLE_SPOTS를 액센트 손톱 두
 * 개(중지·약지)가 accent 패널의 포컬 모티프(하트/별똥별)를 똑같이 중앙에서
 * 따내도록 좁혀, 세트 조화 규칙("액센트 두 개가 같은 포컬 포인트를 공유")이
 * 실제로 지켜지게 한 것.
 *
 *   npx tsx scripts/generate-nail-candidates-v6.mts          (2개 후보 + 콘택트시트 + 게이트1 시트)
 *   npx tsx scripts/generate-nail-candidates-v6.mts v6-1     (특정 후보만)
 */
...
const CANDIDATES: Candidate[] = [
  {
    id: 'v6-1',
    label: '천사날개 글레이즈 — 오로라 펄 베이스 + 은장 깃털 라인 메아리 + 딥 로즈와인 액센트(중지·약지, 날개-하트 포컬)',
  ...
  {
    id: 'v6-2',
    label: '별똥별 글레이즈 — 오로라 펄 베이스 + 은장 유성 라인 메아리 + 딥 미드나잇 라벤더 액센트(중지·약지, 별똥별 포컬)',
...
// 세트 조화: base 세 손톱(엄지·검지·소지)은 텍스처 서로 다른 위치를 짚어
// 자연스러운 변주를 주되(완전 동일 크롭이면 스탬프처럼 반복돼 보임), accent
// 두 손톱(중지·약지)은 같은 중앙 영역(포컬 모티프)을 짚어 "같은 포컬 포인트를
// 공유하는 한 쌍"으로 읽히게 한다.
// base 패널은 모서리(엄지=좌상, 소지=우하 쪽)에 큰 깃털/하트/달/스크롤 같은
// 잡동사니가 몰려 있을 수 있어(모델이 종종 지시를 넘어 장식함), 크기를 줄이고
// 중앙 대각선 라인 밴드에 가깝게 당겨 그 잡동사니를 피한다.
...
async function buildContactSheets(W: number, H: number): Promise<void> {
  ...
  await fullCanvas.composite(fullComposites).png().toFile(resolve(OUT_DIR, 'contact-sheet-v6.png'));
  console.log(`전체 사이즈 콘택트시트 저장: ${resolve(OUT_DIR, 'contact-sheet-v6.png')}`);
  ...
  await sharp({ create: { width: smallW * paths.length, height: smallH + smallLabelH, channels: 4, background: { r: 20, g: 20, b: 20, alpha: 1 } } })
    ...
    .png()
    .toFile(resolve(OUT_DIR, 'contact-sheet-v6-small.png'));
  console.log(`소형 사이즈 콘택트시트 저장: ${resolve(OUT_DIR, 'contact-sheet-v6-small.png')}`);
}
...
async function run() {
  ...
  for (let i = 0; i < NAIL_MASKS.length; i++) {
    // 손톱별로 개별 렌더(전체를 한 번에 그리면 여러 손톱이 한 알파버퍼에 섞여
    // compositeOneNail의 손톱별 재조명 통계 계산이 어긋난다) — nail-masks.mts의
    // renderNailMaskAlpha는 전체 5개를 한 번에 그리므로, 여기서는 손톱 1개짜리
    // 임시 다각형 SVG를 직접 그린다.
    const m = NAIL_MASKS[i];
    const pts = m.points.map((p) => `${p[0]},${p[1]}`).join(' ');
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}"><rect width="${W}" height="${H}" fill="black"/><polygon points="${pts}" fill="white"/></svg>`;
  ...
  console.log('\n모든 후보 생성 완료. ref/nail-candidates/v6-*.webp와 contact-sheet-v6*.png를 육안으로 확인할 것.');
}
