 ...
 * 오버레이를 원본 손(hand.webp)이 아니라 실제로 합성된 결과(v6r3-1.webp)
 ...
 *
 *   npx tsx scripts/debug-shape-check-v6r3.mts v6r3-1
 */
...
async function run() {
  const id = process.argv[2] || 'v6r3-1';
  ...
  await sharp({ create: { width: sheetW, height: sheetH, channels: 4, background: { r: 20, g: 20, b: 20, alpha: 1 } } })
    ...
    .png()
    .toFile(resolve(OUT_DIR, 'v6r3-shape-check.png'));
  console.log('저장 완료:', resolve(OUT_DIR, 'v6r3-shape-check.png'));
}
