/**
 * 히어로 네일아트 "6r2" 라운드(마스크 지오메트리 2차 보정) — 컨트롤러가
 * v6r-2-fingertip-zoom.png 확대 검수에서 재기각한 두 결함을 고친다(6r의
 * 각진 모서리·커버리지 문제는 이미 해결된 상태에서 나온 잔여 결함).
 *
 *   1. 윤곽이 울퉁불퉁한 구름처럼 흔들림(트레이싱 노이즈가 Catmull-Rom에
 *      그대로 흡수됨) — nail-masks.mts에 스플라인 조밀화 *이전* 순환
 *      라플라시안 통제점 스무딩(smoothControlPoints)을 추가해 해결.
 *   2. 약지 큐티클 쪽 페더가 에어브러시처럼 넓게 번짐 — 균일 블러 대신
 *      nail-masks.mts의 renderNailFeatherAlpha로 첨단/큐티클 쪽을 서로 다른
 *      시그마로 렌더링 후 y축 진행률로 블렌드(첨단 2.0px 유지, 큐티클
 *      1.5px로 타이트하게).
 *
 * 텍스처/디자인/작곡 규칙은 generate-nail-candidates-v6r.mts와 완전히 동일하게
 * 재사용한다(같은 /tmp/nailwork/v6-glaze-textures 텍스처, 같은 SAMPLE_SPOTS,
 * 같은 재조명 로직) — 바뀐 것은 오직 마스크 지오메트리·페더뿐이다.
 *
 *   npx tsx scripts/generate-nail-candidates-v6r2.mts            (2개 후보 + 콘택트시트)
 *   npx tsx scripts/generate-nail-candidates-v6r2.mts v6r2-1     (특정 후보만)
 */
...
const CANDIDATES: Candidate[] = [
  {
    id: 'v6r2-1',
    label: '천사날개 글레이즈(6r2 마스크 보정) — 오로라 펄 베이스 + 은장 깃털 라인 메아리 + 딥 로즈와인 액센트(중지·약지, 날개-하트 포컬)',
  ...
  {
    id: 'v6r2-2',
    label: '별똥별 글레이즈(6r2 마스크 보정) — 오로라 펄 베이스 + 은장 유성 라인 메아리 + 딥 미드나잇 라벤더 액센트(중지·약지, 별똥별 포컬)',
...
async function buildContactSheets(W: number, H: number): Promise<void> {
  ...
  await fullCanvas.composite(fullComposites).png().toFile(resolve(OUT_DIR, 'contact-sheet-v6r2.png'));
  console.log(`전체 사이즈 콘택트시트 저장: ${resolve(OUT_DIR, 'contact-sheet-v6r2.png')}`);
  ...
  await sharp({ create: { width: smallW * paths.length, height: smallH + smallLabelH, channels: 4, background: { r: 20, g: 20, b: 20, alpha: 1 } } })
    ...
    .png()
    .toFile(resolve(OUT_DIR, 'contact-sheet-v6r2-small.png'));
  console.log(`소형 사이즈 콘택트시트 저장: ${resolve(OUT_DIR, 'contact-sheet-v6r2-small.png')}`);
}
...
async function run() {
  ...
  // 손톱별 알파 마스크 — 6r2부터는 균일 블러 대신 nail-masks.mts의
  // renderNailFeatherAlpha로 방향성 페더를 적용한다: 첨단(free edge)은
  // 2.0px로 부드럽게(6r 그대로), 큐티클 쪽은 1.5px로 타이트하게(6r2 게이트
  // 재기각 사유 2 — 약지 큐티클 쪽이 에어브러시처럼 넓게 번지던 문제 보정).
  ...
  console.log('\n모든 후보(6r2) 생성 완료. ref/nail-candidates/v6r2-*.webp와 contact-sheet-v6r2*.png를 육안으로 확인할 것.');
}
